...
eosinophil <- summarySE(uv_cells, 
                        measurevar = "Eosinophil",
                        groupvars = c("UV", "Pigmentation"))

# Definir posição de linhas e pontos no gráfico
pd <- position_dodge(0.1)

eosinophil %>% 
  ggplot(aes(x = UV, y = Eosinophil, colour = Pigmentation,
             group = Pigmentation, fill = Pigmentation))+
  geom_errorbar(aes(ymin=Eosinophil-se, ymax=Eosinophil+se), 
...
redmale_summ <- summarySE(redmale, 
                        measurevar = "Red",
...
redmale_summ %>% 
  ggplot(aes(x = Time, y = Red, colour = Treatment,
             group = Treatment, fill = Treatment))+
  geom_errorbar(aes(ymin=Red-se, ymax=Red+se), 
                width=.1, size = 1.1, position=pd) +
  geom_line(position=pd, size = 1.1) +
  geom_point(pch = 21, colour = "black", position=pd, size=3.5)+
  scale_colour_manual(values = c("darkorange", "cyan4")) +
...
## Gráfico
escuridao <- summarySE(darknessmale, 
                        measurevar = "Darkness",
                        groupvars = c("Time", "Treatment"))
...
escuridao %>% 
  ggplot(aes(x = Time, y = Darkness, colour = Treatment,
             group = Treatment, fill = Treatment))+
  geom_errorbar(aes(ymin=Darkness-se, ymax=Darkness+se), 
                width=.1, size = 1.1, position=pd) +
  geom_line(position=pd, size = 1.1) +
  geom_point(pch = 21, colour = "black", position=pd, size=3.5)+
  scale_colour_manual(values = c("darkorange", "cyan4")) +
