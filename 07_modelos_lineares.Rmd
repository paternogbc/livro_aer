...
-   A homegeneidade da variância utiliza um gráfico dos resíduos (eixo X) pelos valores preditos da variável resposta (eixo Y) (Figura \@ref(fig:fig-homo-norm)A). A distribuição dos resíduos será homogênea se não observarmos nenhum padrão na distribuição dos pontos (i.e. forma em V, U ou funil). 
...
7.5 Avalie se existe relação entre a abundância de uma espécie de roedor com o tamanho da área dos fragmentos florestais e/ou a altitude. Faça uma regressão múltipla. Em seguida, crie diferentes modelos e selecione o mais parcimonioso com base no valores do teste de *Likelihood-ratio test* (LRT) e *Akaike information criterion* (AIC). Qual a sua interpretação? Use os dados `Cap7_exercicio5` disponível no pacote `ecodados`.
...
Você pode conferir as soluções dos exercícios [aqui](https://exercicios-livro-aer.netlify.app/cap.-7---modelos-lineares.html).
