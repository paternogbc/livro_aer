...
**7.5**
Avalie se existe relação entre a abundância de uma espécie de roedor com o tamanho da área dos fragmentos florestais e/ou a altitude. Faça uma regressão múltipla. Em seguida, crie diferentes modelos e selecione o mais parcimonioso com base no valores do teste de *Likelihood-ratio test* (LRT) e *Akaike information criterion* (AIC). Qual a sua interpretação? Use os dados `Cap7_exercicio5` disponível no pacote `ecodados`.
