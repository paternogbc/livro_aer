...
Vamos para os comandos da análise do Teste T amostragens indenpendentes e variâncias iguais.
...
Os resíduos não apresentam distribuição homogênea. Portanto, vamos realizar o Teste T com variâncias diferentes. Para isso, use o argumento `var.equal = FALSE`
...
![Exemplo de correlações negativa (A), positiva (B) e nula (C) e variáveis que não apresentam relações lineares entre si (D-E).](img/Correlacao.jpg)
...
Cálculo do Teste de Correlação de Pearson. Para outros testes de correlação como Kendall ou Spearman é só alterar na linha de comando a opção `method` e inserir o teste desejado.
...
-   Recomendamos aos interessados os livros: i) Zar [-@zar_biostatistical_2010] Biostatiscal analysis; ii) Gotelli & Ellison [-@gotelli_primer_2013] A primer of ecological statistics; e iii) Quinn & Keough [-@quinn_experimental_2002] Experimental design and data analysis for biologists.
