...
Precisamos verificar se o agrupamento reduziu a dimensionalidade da matriz de forma eficiente, de maneira a não distorcer a informação. Fazemos isso calculando o **Coeficiente de Correlação Cofenética**.
...
Esta fórmula do *pseudo-F* é específica para desenho experimental com um fator. Outros desenhos mais complexos são apresentados em Anderson [-@anderson_new_2001; -@anderson_permutational_2017]. O cálculo do valor de probabilidade é realizado por métodos de permutação que são discutidos em Anderson & Ter Braak [-@anderson_permutation_2003].
...
Vamos verificar a correlação entre a distância geográfica em linha reta e a dissimilaridade ambiental (Figura \@ref(fig:fig-mantel))
...
Vamos verificar a correlação entre a distância geográfica em linha reta e a dissimilaridade na composição de espécies (Figura \@ref(fig:fig-mantel1))
```{r fig-mantel1, fig.cap="Relação das matrizes de (dis)similaridade dos dados."}
## Mantel
...
matrix.dist.bocaina <- data.frame(x = melt(as.matrix(log(Dist.km)))$value, 
                              y = melt(as.matrix(log(1-dissimil.bocaina)))$value)
...
Vamos verificar a correlação entre a dissimilaridade ambiental e a dissimilaridade na composição de espécies (Figura \@ref(fig:fig-mantel2))
```{r fig-mantel2, fig.cap="Relação das matrizes de (dis)similaridade dos dados."}
## Mantel
...
A principal pergunta do artigo citado acima foi testar se a composição de espécies que obtemos em campo e de coleção científicas é diferente. Isso tem implicações para levantamentos de fauna, já que podemos complementar um tipo de dado com outro ou saber se eles são redundantes. Aqui, `grupos` é apenas um fator que indica se aquela linha da matriz apresenta dados obtidos em campo ou coleção. Vamos testar a hipótese nula de que não há diferença na composição entre as duas fontes de dados ajustando um modelo com distribuição binomial negativa (Figura \@ref(fig:fig-mvabund)).
...
[Soluções dos exercícios](https://exercicios-livro-aer.netlify.app/cap.-9---an%C3%A1lises-multivariadas.html).
