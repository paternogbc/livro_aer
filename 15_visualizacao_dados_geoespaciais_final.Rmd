...
```{r fig-vetor-junção-espacial, fig.cap="Junção espacial da cobertura da terra para as nascentes no município de Rio Claro/SP."}
## Plot
plot(geo_vetor_rio_claro_sirgas2000_utm23s$geom, col = "gray", main = NA, axes = TRUE, graticule = TRUE)
...
```{r fig-vetor-map-plot, fig.cap="Mapa feito com a função `plot()` de um objeto `sf` para os geo_vetor_biomas do Brasil."}
## Plot
...
plot(geo_vetor_biomas$geom, 
     col = c("darkgreen", "orange", "orange4", "forestgreen",
             "yellow", "yellow3"),
     main = "geo_vetor_biomas do Brasil", axes = TRUE, graticule = TRUE)
...
```{r fig-vetor-mapa-ggplot2, fig.cap="Mapa de geo_vetor_biomas do Brasil com o pacote `ggplot2`."}
## Plot
mapa_vetor_biomas_ggplot2 <- ggplot(data = geo_vetor_biomas) +
  ...
  labs(title = "geo_vetor_biomas do Brasil", fill = "Legenda", x = "Longitude", y = "Latitude") +
  theme_bw() +
...
```{r fig-vetor-mapa-tmap, fig.cap="Mapa de geo_vetor_biomas do Brasil com o pacote `tmap`."}
## Mapa
mapa_vetor_biomas_tmap <- tm_shape(geo_vetor_biomas, bbox = c(-74, -35, -27, 10)) +
  ...
  tm_credits("CRS: SIRGAS2000/Geo", position = c(.63, .13)) +
  tm_credits("Fonte: IBGE (2019)", position = c(.63, .09)) +
  tm_layout(title = "geo_vetor_biomas do Brasil",
            title.position = c(.25, .95),
            title.size = 1.8,
            title.fontface = "bold",
            legend.frame = TRUE,
...
knitr::include_graphics("img/cap15_fig78.gif")
```
...
mapa_srtm_rio_claro_mapview_int
```
...
mapa_srtm_rio_claro_leaflet_int
```
...
```{r eval=FALSE}
## Importar os dados pelo pacote ecodados
ecodados::geo_dados_agua
ecodados::geo_dados_banheiro
ecodados::geo_dados_lixo
ecodados::geo_dados_luz
```
...
```{r}
## Visualizar os dados
dplyr::glimpse(geo_dados_agua)
dplyr::glimpse(geo_dados_banheiro)
dplyr::glimpse(geo_dados_lixo )
dplyr::glimpse(geo_dados_luz)
```
...
```{r}
## Filtro e seleção
geo_dados_agua_sel <- geo_dados_agua %>% 
...
geo_dados_banheiro_sel <- geo_dados_banheiro %>% 
...
## Combinar os dados
geo_dados_combinados <- dplyr::bind_cols(geo_dados_agua_sel, 
                            sem_banheiro = geo_dados_banheiro_sel$sem_banheiro, 
                            lixo_coletado = geo_dados_lixo_sel$lixo_coletado, 
                            luz = geo_dados_luz_sel$luz)
dplyr::glimpse(geo_dados_combinados)
```
...
```{r eval=FALSE}
## Importar os dados pelo pacote ecodados
ecodados::geo_vetor_brasil_municipios
```
...
```{r}
## Junção
geo_vetor_brasil_municipios_dados <- dplyr::left_join(
  x = geo_vetor_brasil_municipios, y = geo_dados_combinados, by = "code_muni")

## Dados
dplyr::glimpse(geo_vetor_brasil_municipios_dados)
```
...
```{r message=FALSE}
## Extensão e limite
geo_raster_bioclim_brasil <- geo_raster_bioclim$bio01 %>% 
  raster::crop(geo_vetor_brasil_municipios_dados) %>% 
  raster::mask(geo_vetor_brasil_municipios_dados)
```

```{r fig-raster-bio-br, fig.cap="Adequação do limite da camada BIO01 ao limite do Brasil."}
## Mapa
tm_shape(geo_raster_bioclim_brasil) +
...
```{r}
## Rasterizar
geo_raster_bioclim_brasil_agua_raster <- fasterize::fasterize(
  sf = geo_vetor_brasil_municipios_dados, 
  raster = geo_raster_bioclim_brasil, field = "agua_rede")
geo_raster_bioclim_brasil_sem_banheiro_raster <- fasterize::fasterize(
  sf = geo_vetor_brasil_municipios_dados, 
  raster = geo_raster_bioclim_brasil, field = "sem_banheiro")
geo_raster_bioclim_brasil_lixo_raster <- fasterize::fasterize(
  sf = geo_vetor_brasil_municipios_dados, 
  raster = geo_raster_bioclim_brasil, field = "lixo_coletado")
geo_raster_bioclim_brasil_luz_raster <- fasterize::fasterize(
  sf = geo_vetor_brasil_municipios_dados, 
  raster = geo_raster_bioclim_brasil, field = "luz")
```

Vamos fazer os mapas para ver os resultados (\@ref(fig:fig-raster-bio-br-vars)).

```{r fig-raster-bio-br-vars, fig.cap="Rasterização dos polígonos dos municípios usando o raster da camada BIO01."}
## Mapas
mapa_agua <- tm_shape(geo_raster_bioclim_brasil_agua_raster) +
  tm_raster(pal = "Blues", title = "Água de rede geral (%)") +
  tm_layout(legend.position = c("left", "bottom"))

mapa_banheiro <- tm_shape(geo_raster_bioclim_brasil_sem_banheiro_raster) +
  tm_raster(pal = "RdYlGn", title = "Sem banheiro (%)") +
  tm_layout(legend.position = c("left", "bottom"))

mapa_lixo <- tm_shape(geo_raster_bioclim_brasil_lixo_raster) +
  tm_raster(pal = "Greys", title = "Lixo coletado (%)") +
  tm_layout(legend.position = c("left", "bottom"))

mapa_luz <- tm_shape(geo_raster_bioclim_brasil_luz_raster) +
  tm_raster(pal = "Oranges", title = "Luz (%)") +
  tm_layout(legend.position = c("left", "bottom"))

## Combinar os mapas
tmap_arrange(mapa_agua, mapa_banheiro, mapa_lixo, mapa_luz)
```
...
```{r eval=FALSE}
## Stack
geo_raster_bioclim_brasil_combinado <- raster::stack(geo_raster_bioclim_brasil_agua_raster, 
                                                  geo_raster_bioclim_brasil_sem_banheiro_raster, 
                                                  geo_raster_bioclim_brasil_lixo_raster, 
                                                  geo_raster_bioclim_brasil_luz_raster)

## Exportar
raster::writeRaster(x = geo_raster_bioclim_brasil_combinado, 
                    filename = "var",
